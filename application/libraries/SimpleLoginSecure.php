...
class SimpleLoginSecure {
     ...
     * @return	bool
     */
    function create($user_name = '', $user_email = '', $user_username = '', $user_password = '', $auto_login = true) {
        $this->CI = & get_instance();



        //Make sure account info was sent
        if ($user_username == '' OR $user_email == '' OR $user_password == '') {
            return false;
        }
        ...
        if ($query->num_rows() > 0) //user_username already exists
            return false;
        ...
        //Hash user_password using phpass
        ...
        if ($auto_login)
            $this->login($user_username, $user_password);
     ...
     * @return	bool
     */
    function update($user_id = null, $user_name = '', $user_email = '', $user_username = '', $auto_login = true) {
        $this->CI = & get_instance();

        //Make sure account info was sent
        if ($user_id == null OR $user_username == '' OR $user_email == '') {
            return false;
        }
        ...
        if ($auto_login) {
            ...
            $user_data['user'] = $user_data['user_username']; // for compatibility with Simplelogin
    ...
    function login($user_username = '', $user_password = '') {
        ...
        if ($query->num_rows() > 0) {
            ...
            if (!$hasher->CheckPassword($user_password, $user_data['user_password']))
                return false;
     ...
     * @return  bool
     */
    function edit_password($user_username = '', $old_pass = '', $new_pass = '') {
        $this->CI = & get_instance();
        // Check if the password is the same as the old one
        $this->CI->db->select('user_password');
        $query = $this->CI->db->get_where($this->user_table, array('user_username' => $user_username));
        $user_data = $query->row_array();
        ...
        $user_password_hashed = $hasher->HashPassword($new_pass);
